...
///  这个运算符将错误传递给了 from 函数，这个函数是 std::convert::From trait 中定义的，
///  其是用来将一种错误转换为另一种错误的,当我们需要返回单个错误类型来代表所有可能
///  的失败的时候很有用。只要这些错误类型都实现了转换为所返回的错误类型的 from 函数，转换将被自动被执行
...
fn last_char_of_lastline(text: &str) -> Option<char> {
