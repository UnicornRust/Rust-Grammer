...
在下面的示例中，
 > 这里将 r 的生命周期标记为 'a  x 的生命周期被标记为 'b,
   ...
   因为生命周期 'b 比生命周期 'a 要小，
   被引用的对象比它的引用者存在的时间更短 > 因此这段代码是无法编译的
{
    let r;                // ---------+-- 'a
                          //          |
    {                     //          |
...
单个的生命周期注解本身没有什么意义，因为生命周期注解告诉 Rust 多个引用的
泛型生命周期参数如何相互联系，
...
// 就向泛型参数需要声明在函数名与参数列表之间的`<>`中一样，生命周期参数
...
// 这个约束条件的值都将被借用检查器拒绝，而 longest 函数也并不知道 x 和 y 具体会
// 存在多久，而只需要知道有某个可以被 'a 替代的作用域将会满足这个签名
//
// 函数中使用生命周期注解的时候，这些注解在函数签名中，不存在于函数体的任何代码中
// 生命周期注解成为了函数约定的一部分，非常像签名中的类型,这让编译器的工作变得简单
...
///      说明是个对象的方法，那么输出生命周期参数被赋予 self 的生命周期.
///
/// -------------------------------------------------------------------
///
...
/// 之后被使用，因为这些生命周期是结构体类型的一部分。
/// impl 块中的方法签名中，引用可能与结构体中的引用相关联，也可能是独立的。
///
...
// 上述 impl 块中， impl 之上的生命周期参数是必要的，不过因为第三条生命周期规则，
