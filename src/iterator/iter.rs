...
///     - 当迭代器结束时则返回 None. 
///
///  可以通过调用 迭代器的 next() 方法来获得迭代器的下一个元素。
/// -------------------------------------------------------------------------------
///  迭代器的种类：
///   - `iter()` : 获取到的是原数据的不可变引用, 不可变引用的迭代器
///   - `into_iter()` : 使用这个方法将返回拥有所有权的迭代器。
///   - `iter_mut()` : 可获得迭代可变引用的迭代器
fn iter_demonstration() {
    let v1 = [1, 2, 3];
    // 需要注意的是：当需要调用 迭代器 next() 方法时，next() 改变了迭代器中用来记录
...
// sum 
//  - 获取迭代器的所有权, 调用 sum 之后不再允许使用迭代器
//  - 通过反复调用 next, 遍历所有元素
//  - 每次迭代元素，把当前元素添加到一个总和里，迭代结束，返回总和
//
fn iter_sum() {
    let v1 = vec![1, 3, 5];
    let v1_iter = v1.iter();
...
/// > Iterator trait 中还定义了另一类方法，被称为 `迭代器适配器(Iterator adaptors)` ,
